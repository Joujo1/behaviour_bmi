...
ax1.plot(t_sec / 3600, drift_fit_ms, color="red", linewidth=1.5, linestyle="--",
         label=f"Linear fit  ({drift_us_per_s:.3f} ppm,  {drift_us_per_s*86400/1e3:.1f} ms/24 h)")
...
# Panel 2: residuals after removing linear drift
ax2 = fig.add_subplot(gs[1])
ax2.plot(t_sec / 3600, residuals_us, linewidth=1, color="orange")
ax2.axhline(0, color="black", linewidth=0.8, linestyle="--")
ax2.fill_between(t_sec / 3600, residuals_us, alpha=0.2, color="orange")
ax2.set_xlabel("Elapsed time (hours)")
ax2.set_ylabel("Residual (µs)")
ax2.set_title(f"Residuals after drift removal  (std = {residuals_us.std():.3f} µs)")
